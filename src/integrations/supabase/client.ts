...
export const updateProduct = async (productId: string, updates: Partial<Database['public']['Tables']['products']['Update']>) => {
  // First check if there are any actual changes to apply
  ...
  if (fetchError) {
    console.error(`Error fetching current product with ID ${productId}:`, fetchError);
    throw fetchError;
  }
  ...
  // If no changes, return early with a flag
  if (!hasChanges) {
