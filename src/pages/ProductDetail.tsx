...
const ProductDetail = () => {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white">
      ...
      {/* Only show tab content when a tab is clicked - without the tabs selector */}
      {showTabContent && (
          ...
          <div className="p-4 bg-white">
            {activeTab === "description" && (
              <div>
                ...
                <p className="text-sm text-gray-600">{productForTabs.description}</p>
            ...
            {activeTab === "reviews" && (
              <div>
                ...
                <div className="flex items-center mb-4">
                  <div className="mr-4">
                    <div className="text-3xl font-bold">{productForTabs.rating}</div>
                    <div className="flex items-center">
                      {[...Array(5)].map((_, i) => (
                        <svg key={i} className={`w-4 h-4 ${i < Math.floor(productForTabs.rating) ? 'text-yellow-400' : 'text-gray-300'}`} fill="currentColor" viewBox="0 0 20 20">
                          <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118l-2.8-2.034c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path>
                        </svg>
                      ))}
                    </div>
                    <div className="text-xs text-gray-500">{productForTabs.reviewCount} reviews</div>
                  </div>
                  <div className="flex-1">
                    {[5, 4, 3, 2, 1].map((star) => (
                      <div key={star} className="flex items-center">
                        <span className="text-xs w-3 text-gray-600">{star}</span>
                        <div className="mx-2 h-2 flex-1 bg-gray-200 rounded-full">
                          <div className="h-2 bg-yellow-400 rounded-full" style={{ width: `${star === 5 ? 70 : star === 4 ? 20 : star === 3 ? 5 : star === 2 ? 3 : 2}%` }}></div>
                        </div>
                        <span className="text-xs text-gray-600">{star === 5 ? 70 : star === 4 ? 20 : star === 3 ? 5 : star === 2 ? 3 : 2}%</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  {[
                    { name: "Emma S.", rating: 5, date: "2 weeks ago", comment: "Absolutely love this product! It exceeded my expectations in every way." },
                    { name: "Michael T.", rating: 4, date: "1 month ago", comment: "Great product overall, but there's room for improvement with the battery life." },
                    { name: "Sarah W.", rating: 5, date: "2 months ago", comment: "The best purchase I've made this year. Worth every penny!" }
                  ].map((review, index) => (
                ...
                <Button className="w-full mt-4 bg-gray-100 hover:bg-gray-200 text-gray-700 font-normal">
                  View All {productForTabs.reviewCount} Reviews
                </Button>
              </div>
            )}
          </div>
