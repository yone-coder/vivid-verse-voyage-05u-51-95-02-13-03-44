...
import { 
  ...
  Globe, AlertCircle, Fingerprint, Bell, Shield, Activity, 
  User, TrendingUp, Key, Sliders, Zap, Coffee, HelpCircle
} from 'lucide-react';
...
export default function UltraModernLogin() {
  ...
  const inputBgClasses = theme === 'dark'
    ? 'bg-gray-800 border-gray-700 focus:border-blue-500'
    : 'bg-gray-50 border-transparent focus:border-orange-500';
      ...
      <div className="w-full flex flex-col">
        ...
        <div className="max-w-4xl mx-auto w-full px-4 sm:px-6 py-8 relative">
          ...
          <div className="absolute top-4 right-4 flex items-center space-x-3">
            ...
            >
              {theme === 'dark' ? <Coffee className="h-5 w-5" /> : <Coffee className="h-5 w-5" />}
            </button>
            <button 
            ...
            <div className={`absolute right-4 top-16 z-10 p-4 rounded-lg shadow-xl border ${theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} w-64`}>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="relative inline-block w-10 align-middle select-none">
                    <input type="checkbox" checked className="absolute opacity-0 w-0 h-0" />
            ...
            <div className={`p-4 mb-4 rounded-lg border ${
              ...
              <ul className="space-y-2 text-sm">
                <li className="flex items-start">
                  <span className={`p-0.5 rounded-full mr-2 mt-0.5 ${theme === 'dark' ? 'bg-blue-400' : 'bg-orange-500'}`}>
          ...
          {/* Safety notice */}  
          <div className="mt-6 p-3 bg-blue-50 flex items-start border border-blue-100 rounded-lg">  
            <AlertCircle className="h-5 w-5 text-blue-500 mt-0.5 mr-3 flex-shrink-0" />  
            <p className="text-xs text-blue-700">  
              For account security, never share your password or verification codes. Representatives will never ask for this information.  
            </p>  
          </div>  
        </div>  
        ...
        <div className="fixed bottom-6 right-6">  
          <button className={`rounded-full p-3 shadow-lg flex items-center justify-center hover:scale-110 transform active:scale-95 transition-all ${
            theme === 'dark' ? 'bg-gray-800 text-blue-400 hover:bg-gray-700' : 'bg-white text-orange-500 hover:bg-orange-50'
          }`}>  
        ...
        <div className="fixed bottom-6 left-6 sm:flex hidden">  
          <button className={`flex items-center rounded-full pl-3 pr-4 py-2 shadow-lg hover:shadow-xl transition-all ${
            theme === 'dark' ? 'bg-gray-800 hover:bg-gray-700' : 'bg-white hover:bg-gray-50'
          }`}>  
            <div className={`p-1 rounded-full mr-2 ${theme === 'dark' ? 'bg-blue-500' : 'bg-orange-500'}`}>  
              <Smartphone className="h-4 w-4 text-white" />  
            </div>  
            <span className={`text-sm font-medium ${theme === 'dark' ? 'text-gray-200' : 'text-gray-700'}`}>Get our app</span>  
          </button>  
        </div>  
      </div>
    </div>
