...
export default function AliExpressHeaderCompressed() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 flex flex-col bg-white shadow-sm" style={{ height: '44px' }}>
      {/* Top Bar (compressed) */}
      <div className="flex items-center justify-between px-2" style={{ height: '28px' }}>
        <Logo width={20} height={20} className="text-orange-500" />
        ...
        <div className="flex flex-1 mx-2" ref={searchRef}>
          <div
            ...
            style={{ height: '20px' }}
          >
            <Search className="ml-1 h-3 w-3 text-orange-500" />
            <input
              className="px-1 text-[10px] bg-gray-100 outline-none placeholder-gray-400 w-full"
              placeholder="Search"
              value={searchQuery}
            ...
            {searchQuery && (
              <div
                className="cursor-pointer mr-1 rounded-full hover:bg-gray-200"
        ...
        <div className="flex items-center space-x-1 flex-shrink-0">
          <div className="cursor-pointer relative hover:bg-gray-100 p-1 rounded-full">
            ...
            <span className="absolute -top-1 -right-1 bg-orange-500 text-white text-[8px] rounded-full h-3 w-3 flex items-center justify-center">
              2
            </span>
          </div>
      ...
      {/* Tabs (compressed) */}
      <div className="flex overflow-x-auto no-scrollbar" style={{ height: '16px' }}>
        {categories.map((category) => (
          <button
            key={category}
            className={`whitespace-nowrap px-2 text-[10px] leading-[16px] font-medium transition-all border-b-2 ${
              activeTab === category
