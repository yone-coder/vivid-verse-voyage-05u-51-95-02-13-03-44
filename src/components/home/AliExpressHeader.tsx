...
export default function AliExpressHeaderWithStates() { const { progress } = useScrollProgress(); const [isOpen, setIsOpen] = useState(false); const [activeTab, setActiveTab] = useState('All'); const [isSearchFocused, setIsSearchFocused] = useState(false); const [searchQuery, setSearchQuery] = useState(''); const [voiceSearchActive, setVoiceSearchActive] = useState(false); const searchRef = useRef(null);

const categories = ['All', 'Women', 'Men', 'Electronics', 'Home', 'Beauty', 'Kids', 'Sports'];

useEffect(() => { const handleClickOutside = (event) => { if (searchRef.current && !searchRef.current.contains(event.target)) { // Click outside logic (optional) } }; document.addEventListener('mousedown', handleClickOutside); return () => document.removeEventListener('mousedown', handleClickOutside); }, []);

const handleSearchFocus = () => setIsSearchFocused(true); const handleClearSearch = () => setSearchQuery(''); const handleVoiceSearch = () => setVoiceSearchActive(!voiceSearchActive);

return ( <header className="fixed top-0 left-0 right-0 z-50 transition-all duration-700" style={{ height: '44px' }} > <div className="flex items-center justify-between px-1 transition-all duration-700" style={{ height: '44px', backgroundColor: rgba(255, 255, 255, ${progress * 0.95}), backdropFilter: blur(${progress * 8}px), backgroundImage: progress < 0.5 ? 'linear-gradient(to bottom, rgba(0,0,0,0.7), rgba(0,0,0,0))' : 'none', }} > <div className="flex items-center"> {progress < 0.5 ? ( <button onClick={() => setIsOpen(!isOpen)} className="tab-button flex items-center space-x-1 py-1 px-3 text-xs font-medium bg-black bg-opacity-30 text-white rounded-full" style={{ height: '28px' }} > <span>{activeTab}</span> <ChevronDown className={w-3.5 h-3.5 ${isOpen ? 'rotate-180' : ''}} /> </button> ) : ( <div className="mr-1"> <Logo width={28} height={28} className="text-orange-500" /> </div> )} </div>

{progress >= 0.5 && (
      <div className="flex-1 max-w-xs mx-2 relative" ref={searchRef}>
        <div
          className={`flex items-center bg-gray-100 rounded-full border ${
            isSearchFocused ? 'border-orange-500' : 'border-gray-200'
          }`}
        >
