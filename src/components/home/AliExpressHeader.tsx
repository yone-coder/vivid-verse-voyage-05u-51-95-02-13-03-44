...
useEffect(() => { const handleClickOutside = (event) => { if (searchRef.current && !searchRef.current.contains(event.target)) { // Placeholder for click outside logic } }; document.addEventListener('mousedown', handleClickOutside); return () => document.removeEventListener('mousedown', handleClickOutside); }, []);
...
return ( <header className="fixed top-0 left-0 right-0 z-50 flex flex-col"> {/* Main Header */} <div className="flex items-center justify-between px-1 transition-all duration-700" style={{ height: '44px', backgroundColor: rgba(255, 255, 255, ${progress * 0.95}), backdropFilter: blur(${progress * 8}px), backgroundImage: progress < 0.5 ? 'linear-gradient(to bottom, rgba(0,0,0,0.7), rgba(0,0,0,0))' : 'none', }} > <div className="flex items-center"> {progress < 0.5 ? ( <button
onClick={togglePanel}
className="tab-button flex items-center justify-center space-x-1 py-1 px-3 font-medium text-sm bg-black bg-opacity-30 text-white rounded-full"
      ...
      {progress < 0.5 ? (
        <>
          ...
          <div className="cursor-pointer relative bg-black bg-opacity-40 p-1.5 rounded-full">
            ...
            <span className="absolute -top-1 -right-1 bg-orange-500 text-white text-xs rounded-full h-3.5 w-3.5 flex items-center justify-center">2</span>
          </div>
          <div className="cursor-pointer bg-black bg-opacity-40 p-1.5 rounded-full">
