...
const MobileLocalTransferSheetPage: React.FC<MobileLocalTransferSheetPageProps> = ({ 
  ...
  // PayPal integration effect for step 3 (international transfers only)
  useEffect(() => {
    if (currentStep === 3 && transferData.transferType === 'international' && paypalContainerRef.current) {
      ...
      script.innerHTML = `
        (function() {
          ...
          let paypalHostedFieldsLocal = null;
          ...
          let resetPurchaseButton = () => {
              const btn = document.querySelector("#card-form").querySelector("button[type='submit']");
              if (btn) {
                btn.removeAttribute("disabled");
                const buttonText = currentPriceLocal ? \`Pay \${currentPriceLocal.display}\` : "Pay Now";
            ...
            return new Promise(async (resolve, reject) => {
              try {
                const response = await fetch(\`\${API_BASE_URL}/get_client_token\`, {
                  method: "POST", 
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({ "customer_id": customerIdLocal }),
                });
          ...
          let displayErrorAlert = (message = "An error occurred. Please try again.") => {
          ...
          let displaySuccessMessage = (order_details) => {
              const alertsContainer = document.getElementById("alerts");
              if (alertsContainer) {
          ...
          function scriptToHead(attributes_object) {
          ...
          .then(() => {
              ...
              if (window.paypal && window.paypal.HostedFields.isEligible()) {
                  paypalHostedFieldsLocal = window.paypal.HostedFields.render({
                    createOrder: () => {
                      return fetch(\`\${API_BASE_URL}/create_order\`, {
                          ...
                          body: JSON.stringify({ 
                              "intent": intent,
                              "amount": currentPriceLocal ? currentPriceLocal.value : null
                          })
                      })
                          ...
                          .then(() => {
                            ...
                            .then((order_details) => {
                             ...
                             .catch((error) => {
                                ...
                                displayErrorAlert("Payment processing failed. Please try again.");
                                resetPurchaseButton();
                             });
                          })
                          .catch((err) => {
                            ...
                            resetPurchaseButton();
                            displayErrorAlert("Card validation failed. Please check your information.");
                          });
                      });
                    }
                  });
                } else {
                  displayErrorAlert("Card payments are not supported in this browser.");
                }
          })
          .catch((error) => {
              console.error('PayPal initialization error:', error);
              resetPurchaseButton();
              displayErrorAlert("Failed to initialize payment system. Please refresh the page.");
          });
        })();
      `;
  ...
  }, [currentStep, transferData.amount, transferData.transferType]);
  ...
  const handleStickyPayment = async () => {
    if (transferData.transferType === 'national') {
      // Use MonCash for national transfers
      await handleMonCashPayment();
    } else {
      // Use PayPal for international transfers
      setIsPaymentLoading(true);
            ...
            <div className="space-y-6">
              <div className="space-y-3">
                <TransferTypeSelector 
                  transferType={transferData.transferType || 'national'}
                  ...
                  <div className="border-t pt-3">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-medium capitalize">
                        {transferData.transferType === 'national' ? 'MonCash' : 'Credit Card'}
                      </span>
                    </div>
                  </div>
