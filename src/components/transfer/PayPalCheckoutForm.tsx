...
const PayPalCheckoutForm: React.FC<PayPalCheckoutFormProps> = ({
  ...
  useEffect(() => {
    ...
    if (isInitializedRef.current || !paypalContainerRef.current || !transferAmount) {
      return;
    }

    console.log('Initializing PayPal checkout form');
    ...
    script.innerHTML = `
      (function() {
        // Use unique scoped variables to prevent redeclaration
        const paypalContext = {
          customer_id: "",
          order_id: "",
          currentPrice: null,
          paypal_hosted_fields: null
        };
        ...
        const addFormValidationListeners = () => {
          ...
          if (emailInput) {
            ...
            newEmailInput.addEventListener('input', validatePaymentForm);
            newEmailInput.addEventListener('blur', validatePaymentForm);
          }
        ...
        const reset_purchase_button = () => {
          ...
          if (btn) {
            ...
            const buttonText = paypalContext.currentPrice ? \`Pay \${paypalContext.currentPrice.display}\` : "Pay Now";
          ...
          return new Promise(async (resolve, reject) => {
            try {
              const response = await fetch(\`\${API_BASE_URL}/get_client_token\`, {
                ...
                body: JSON.stringify({ "customer_id": paypalContext.customer_id }),
              });
        ...
        const display_error_alert = (message = "An error occurred. Please try again.") => {
        ...
        const display_success_message = (order_details) => {
          const alertsContainer = document.getElementById("alerts");
          if (alertsContainer) {
        ...
        const fetchCurrentPrice = () => {
          ...
          paypalContext.currentPrice = priceData;
        ...
        .then(() => {
          ...
          if (window.paypal && window.paypal.HostedFields.isEligible()) {
            paypalContext.paypal_hosted_fields = window.paypal.HostedFields.render({
              createOrder: () => {
                return fetch(\`\${API_BASE_URL}/create_order\`, {
                  ...
                  body: JSON.stringify({ 
                    "intent": intent,
                    "amount": paypalContext.currentPrice ? paypalContext.currentPrice.value : null
                  })
                })
                .then((response) => response.json())
                .then((order) => { 
                  paypalContext.order_id = order.id; 
                  return order.id; 
                });
              },
              styles: {
                ...
                newCardForm.addEventListener("submit", (event) => {
                  ...
                  .then(() => {
                    ...
                    return fetch(\`\${API_BASE_URL}/complete_order\`, {
                      ...
                      body: JSON.stringify({
                        "intent": intent,
                        "order_id": paypalContext.order_id,
    ...
    return () => {
      console.log('Cleaning up PayPal component');
  ...
  }, [transferAmount]); // Only depend on transferAmount to prevent unnecessary re-renders
