...
const PayPalCardPayment: React.FC<PayPalCardPaymentProps> = ({
  amount,
  onEmailCaptured,
  onPaymentSuccess,
  onPaymentError,
      ...
      script.innerHTML = `
        ...
        const validatePaymentForm = () => {
          ...
          if (emailInput && emailInput.value) {
            window.paypalEmailCaptured && window.paypalEmailCaptured(emailInput.value);
          }
          
          const isValid = isEmailValid && hasCardFields;
          window.paypalFormValidation && window.paypalFormValidation(isValid);
        };
        ...
        let display_error_alert = (message = "An error occurred. Please try again.") => {
            ...
            window.paypalPaymentError && window.paypalPaymentError(message);
        }
        ...
        let display_success_message = (order_details) => {
            ...
            setTimeout(() => {
              window.paypalPaymentSuccess && window.paypalPaymentSuccess(order_details);
            }, 2000);
        }

        // Set up callback functions
        window.paypalEmailCaptured = function(email) {
          ${onEmailCaptured.toString()}(email);
        };

        window.paypalPaymentSuccess = function(orderDetails) {
          ${onPaymentSuccess.toString()}(orderDetails);
        };

        window.paypalPaymentError = function(message) {
          ${onPaymentError.toString()}(message);
        };

        window.paypalFormValidation = function(isValid) {
          ${onFormValidation.toString()}(isValid);
        };
        ...
        function fetchCurrentPrice() {
          const priceData = {
            value: "${amount}",
            display: "$" + parseFloat("${amount}").toFixed(2),
            currency: "USD"
          };
  ...
  }, [amount, onEmailCaptured, onPaymentSuccess, onPaymentError, onFormValidation]);
