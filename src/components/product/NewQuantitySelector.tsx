...
const NewQuantitySelector: React.FC<NewQuantitySelectorProps> = ({
  ...
  const handleQuantityChange = (newQuantity: number) => {
    ...
    const validQuantity = Math.max(1, Math.min(MAX_QUANTITY, newQuantity));
  ...
  const renderMilestoneMarkers = () => {
    ...
    PRICE_TIERS.forEach((tier, index) => {
      if (index < PRICE_TIERS.length - 1) {
        ...
        const percentage = (nextTier.min / MAX_QUANTITY) * 100;
  ...
  return (
    <div className="w-full max-w-md bg-white rounded p-3 relative">
      ...
      <div className="flex items-center gap-3 mb-3">
        <div className="flex border border-gray-300 rounded h-8">
          ...
          >
            <Plus size={16} className={quantity >= MAX_QUANTITY ? "text-gray-300" : "text-gray-700"} />
          </button>
        </div>
      ...
      <div className="mb-4 relative">
        <div className="h-1 bg-gray-200 rounded-full mb-1 relative overflow-hidden">
          <div 
            ...
            style={{ width: `${(quantity / MAX_QUANTITY) * 100}%` }}
          />
