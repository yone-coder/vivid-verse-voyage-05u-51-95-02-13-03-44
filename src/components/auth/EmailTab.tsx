...
return ( <form onSubmit={handleSubmit}> <motion.header className="text-center mb-4" variants={containerVariants} initial="hidden" animate="visible" > <motion.h2 className="text-base font-semibold text-foreground mb-1" variants={childVariants}> Let’s get started </motion.h2> <motion.p className="text-sm text-muted-foreground" variants={childVariants}> Please enter your email to {emailExists === false ? "create your account" : "continue"}. </motion.p> </motion.header> <div className="relative w-full max-w-sm"> <Input ref={inputRef} id="email" type="email" spellCheck="false" autoCorrect="off" autoCapitalize="none" value={email} onChange={(e) => setEmail(normalizeEmail(e.target.value))} onFocus={handleFocus} onBlur={handleBlur} onKeyDown={handleKeyDown} placeholder="john.doe@example.com" className={w-full pl-10 pr-10 h-11 text-sm bg-background transition-all duration-300 ease-in-out rounded-md shadow-sm focus:shadow-md ${ validationMessage || errorMessage ? 'border-destructive focus:border-destructive focus:ring-destructive/20 bg-destructive/5' : isValid ? 'border-green-500 focus:border-green-600 focus:ring-green-200 bg-green-50/30 dark:bg-green-950/10' : focused ? 'border-primary/30 focus:border-primary focus:ring-primary/20' : 'border-input hover:border-input focus:border-input focus:ring-ring/20' }} autoComplete="email" aria-invalid={!!validationMessage || !!errorMessage} aria-describedby={validationMessage ? 'email-validation-error' : undefined} /> {(checking || verifying) && <Loader2 className="h-4 w-4 animate-spin absolute right-3 top-3.5" />} {isValid && !checking && !verifying && !errorMessage && ( <Check className={h-4 w-4 text-green-500 absolute right-3 top-3.5 ${showValidationSuccess ? 'animate-bounce' : 'animate-fadeIn'}} aria-label="Valid email" /> )} {email.length > 0 && ( <button type="button" onClick={clearInput} tabIndex={0} className="absolute right-8 top-2.5 text-muted-foreground hover:text-foreground hover:bg-muted rounded-full p-1 transition-all duration-200 transform hover:scale-110" aria-label="Clear input"> <X className="h-3.5 w-3.5" /> </button> )} {typoSuggestion && ( <button type="button" onClick={applyTypoSuggestion} className="flex items-center gap-1.5 mt-1 text-xs text-primary hover:text-primary/90 hover:underline"> <span>Did you mean: <strong>{typoSuggestion}</strong>?</span> <Check className="h-3 w-3" /> </button> )} {errorMessage && ( <div className="flex items-center justify-center gap-1.5 mt-2 p-2 rounded-md bg-destructive/10 text-destructive text-sm max-w-sm"> <AlertTriangle className="h-4 w-4" /> <span className="font-medium">{errorMessage}</span> </div> )} <AnimatePresence> {validationMessage && !errorMessage && ( <EmailValidationMessage message={validationMessage} /> )} </AnimatePresence> {showSuggestions && suggestions.length > 0 && ( <div ref={suggestionsRef} className="w-full max-w-sm"> <EmailSuggestions
suggestions={suggestions}
hoveredIndex={hoveredIndex}
setHoveredIndex={setHoveredIndex}
selectSuggestion={selectSuggestion}
premiumDomains={premiumDomains}
/> </div> )} {showSubmitButton && ( <button type="submit" disabled={!isValid || checking || verifying} className={w-full max-w-sm mt-4 py-2.5 rounded-lg font-medium transition-all duration-200 relative overflow-hidden ${ isValid && !checking && !verifying ? 'bg-primary hover:bg-primary/90 text-primary-foreground shadow-sm hover:shadow' : 'bg-muted text-muted-foreground cursor-not-allowed' }} > {checking || verifying ? ( <span className="flex items-center justify-center gap-2"> <Loader2 className="h-4 w-4 animate-spin" /> {verifying ? "Verifying..." : "Checking..."} </span> ) : ( <span className="relative z-10">Continue</span> )} </button> )} </div> <style>{@keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } } .animate-fadeIn { animation: fadeIn 0.2s ease-in-out; } @keyframes pulseSuccess { 0%, 100% { transform: scale(1); opacity: 1; } 50% { transform: scale(1.2); opacity: 0.8; } } .animate-pulse-success { animation: pulseSuccess 0.6s ease-in-out; }}</style> </form> ); };
