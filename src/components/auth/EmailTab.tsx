...
import { premiumDomains, normalizeEmail, generateSuggestions, getValidationMessage } from './EmailUtils';
...
const EmailTab = ({ email, setEmail, onSubmit, showSubmitButton = false }: EmailTabProps) => { 
  ...
  const handleBlur = useCallback((e: React.FocusEvent) => { 
    ...
    setTimeout(() => { 
      if (document.activeElement !== inputRef.current && !suggestionsRef.current?.contains(document.activeElement)) { 
        setFocused(false); 
        setShowSuggestions(false); 
      } 
    }, 200); 
  ...
  return (
    <form onSubmit={handleSubmit}> 
      ...
      <div className="relative w-full max-w-sm"> 
        ...
        {(checking || verifying) && (
          <Loader2 className="h-4 w-4 animate-spin absolute right-3 top-3.5" />
        )} 
        
        {isValid && !checking && !verifying && !errorMessage && (
          <Check 
            className={`h-4 w-4 text-green-500 absolute right-3 top-3.5 ${
        ...
        {email.length > 0 && (
          <button 
            ...
            onClick={clearInput} 
            tabIndex={0} 
            className="absolute right-8 top-2.5 text-muted-foreground hover:text-foreground hover:bg-muted rounded-full p-1 transition-all duration-200 transform hover:scale-110" 
